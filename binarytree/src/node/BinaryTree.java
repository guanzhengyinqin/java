...
public class BinaryTree {
	...
	static class BinaryTreeTraversal{
		 ...
		 * 这种方式具备扩展性，它模拟递归的过程，将左子树点不断的压入栈，知道null，然后处理栈顶节点的右子树
		 */
		 ...
		 * 
		 * @param root 树根节点
		 * 利用栈模拟递归过程实现循环中中序遍历的二叉树
		 * 思想和上面的preOrderStack_2相同，只是访问的时间是在左子树都处理完知道null的时候出栈并访问。
		 */
		 ...
		 * 
		 * @param root 树根节点
		 * 后序遍历不同于先序和中序，他是要先处理完左右子树，然后在处理根（回溯），所以需要一个记录那些节点已经被访问的结构（可以在树结构
		 * 里面加一个标记），这里可以用map实现
		 */
		 ...
		 * 
		 * @param root 树根节点
		 * 层序遍历二叉树，用队列实现，先将根节点入队列，只要队列不为空，然后出队列，并访问，接着将访问借点的左右子树依次入队列
		 */
